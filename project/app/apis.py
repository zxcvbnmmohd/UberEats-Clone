...
from app.models import Order, OrderDetails, Business, Item
from app.serializers import OrderDriverSerializer, OrderSerializer, BusinessSerializer, ItemSerializer, OrderStatusSerializer
...
import stripe
from project.settings import STRIPE_API_KEY
...
def customer_add_order(request):
  ...
  if request.method == "POST":
    ...
    # Check if items in only one business and then calculate the order total
    order_total = 0
    for item in order_details:
      if not Item.objects.filter(id=item["item_id"], business_id=request.POST["business_id"]):
        return JsonResponse({"status": "failed", "error": "Items must be in only one business"})
      else:
